package model.zp1ajax;

public class ZpRecord2  {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private String MSA_2;
	private String PID_3_CX_1_1;
	private String PID_3_CX_1_2;
	private String okato2;
	private String IN1_3_CX_1;	
	private String IN1_12;
	private String IN1_13;
	private String IN1_15;
	private String IN1_35;
	private String IN1_36;
	private String qri1;
	private String qri2;
	private String qri3;
	private String qri4;
	private int npp;
	private String dinput;
	private String PID7;
	private String PID29;
	   private String PID8;

		public String getMSA_2() {
		return MSA_2;
	}

	public void setMSA_2(String mSA_2) {
		MSA_2 = mSA_2;
	}

	public String getPID_3_CX_1_1() {
		return PID_3_CX_1_1;
	}

	public void setPID_3_CX_1_1(String pID_3_CX_1_1) {
		PID_3_CX_1_1 = pID_3_CX_1_1;
	}

	public String getPID_3_CX_1_2() {
		return PID_3_CX_1_2;
	}

	public void setPID_3_CX_1_2(String pID_3_CX_1_2) {
		PID_3_CX_1_2 = pID_3_CX_1_2;
	}

	public String getOkato2() {
		return okato2;
	}

	public void setOkato2(String okato2) {
		this.okato2 = okato2;
	}

	public String getIN1_3_CX_1() {
		return IN1_3_CX_1;
	}

	public void setIN1_3_CX_1(String iN1_3_CX_1) {
		IN1_3_CX_1 = iN1_3_CX_1;
	}

	public String getIN1_12() {
		return IN1_12;
	}

	public void setIN1_12(String iN1_12) {
		IN1_12 = iN1_12;
	}

	public String getIN1_13() {
		return IN1_13;
	}

	public void setIN1_13(String iN1_13) {
		IN1_13 = iN1_13;
	}

	public String getIN1_15() {
		return IN1_15;
	}

	public void setIN1_15(String iN1_15) {
		IN1_15 = iN1_15;
	}

	public String getIN1_35() {
		return IN1_35;
	}

	public void setIN1_35(String iN1_35) {
		IN1_35 = iN1_35;
	}

	public String getIN1_36() {
		return IN1_36;
	}

	public void setIN1_36(String iN1_36) {
		IN1_36 = iN1_36;
	}

	public String getQri1() {
		return qri1;
	}

	public void setQri1(String qri1) {
		this.qri1 = qri1;
	}

	public String getQri2() {
		return qri2;
	}

	public void setQri2(String qri2) {
		this.qri2 = qri2;
	}

	public String getQri3() {
		return qri3;
	}

	public void setQri3(String qri3) {
		this.qri3 = qri3;
	}

	public String getQri4() {
		return qri4;
	}

	public void setQri4(String qri4) {
		this.qri4 = qri4;
	}

	public int getNpp() {
		return npp;
	}

	public void setNpp(int npp) {
		this.npp = npp;
	}

	public String getDinput() {
		return dinput;
	}

	public void setDinput(String dinput) {
		this.dinput = dinput;
	}

	public String getPID7() {
		return PID7;
	}

	public void setPID7(String pID7) {
		PID7 = pID7;
	}

	public String getPID29() {
		return PID29;
	}

	public void setPID29(String pID29) {
		PID29 = pID29;
	}

	public String getPID8() {
		return PID8;
	}

	public void setPID8(String pID8) {
		PID8 = pID8;
	}

	@Override
	public String toString() {
		StringBuilder builder = new StringBuilder();
		builder.append(MSA_2);
		builder.append(",");
		builder.append(PID_3_CX_1_1);
		builder.append(",");
		builder.append(PID_3_CX_1_2);
		builder.append(",");
		builder.append(okato2);
		builder.append(",");
		builder.append(IN1_3_CX_1);
		builder.append(",");
		builder.append(IN1_12);
		builder.append(",");
		builder.append(IN1_13);
		builder.append(",");
		builder.append(IN1_15);
		builder.append(",");
		builder.append(IN1_35);
		builder.append(",");
		builder.append(IN1_36);
		builder.append(",");
		builder.append(qri1);
		builder.append(",");
		builder.append(qri2);
		builder.append(",");
		builder.append(qri3);
		builder.append(",");
		builder.append(qri4);
		builder.append(",");
		builder.append(npp);
		builder.append(",");
		builder.append(dinput);
		builder.append(",");
		builder.append(PID7);
		builder.append(",");
		builder.append(PID8);
		builder.append(",");
		builder.append(PID29);
		return builder.toString();
	}

	
		


}
